...
for i in $(ls $SUCCESS_DIR); do          		 ## check every .dat in dat dir
        ...
        if [ $(stat -c %Y $SUCCESS_DIR/$i) -ge $(cat $TSTAMP_DIR/housekeeping_tstamp1.txt | grep $i | cut -f3 -d" ") ]; then ## if current timestamp is greater than equal to listed timestamp then delete the file
...
for i in $(ls $WLNFILTER_SUCCESS); do                          ## check every .dat in dat dir
        ...
        if [ $(stat -c %Y $WLNFILTER_SUCCESS/$i) -ge $(cat $TSTAMP_DIR/timestamp_wln_ftr.txt | grep $i | cut -f3 -d" ") ]; then ## if current timestamp is greater than equal to listed timestamp then delete the file
