...
if [ -d $(ls -A $RECEIVE_DIR/) ]; then
   echo $(date +%F)": No Files have been processed!" >> $LOG_DIR/$NAMING_CONVENTION.log
fi;
        ...
        if [ $(head -n1 $RECEIVE_DIR/$i | cut -f28-30 -d,) == "false,2,true" ]; then
                if [ $(cat $RECEIVE_DIR/$i 2> /dev/null | wc -l) == "1" ]; then
                        if grep -rq $(head -n1 $RECEIVE_DIR/$i | cut -f31 -d,) $CASE2_DIR; then
                                echo $(date +%F)": file $i is a possible success for case2" >> $LOG_DIR/${NAMING_CONVENTION}.log;
                                mv $RECEIVE_DIR/$i $POSSIBLE_SUCCESS;
                        elif grep -rq $(head -n1 $RECEIVE_DIR/$i | cut -f31 -d,) $CASE5_DIR; then
                                echo $(date +%F)": file $i is a possible success for case5" >> $LOG_DIR/${NAMING_CONVENTION}.log;
                                mv $RECEIVE_DIR/$i $POSSIBLE_SUCCESS;
                        elif grep -rq $(head -n1 $RECEIVE_DIR/$i | cut -f31 -d,) $CASE6_DIR; then
                                echo $(date +%F)": file $i is a possible success for case6" >> $LOG_DIR/${NAMING_CONVENTION}.log;
                                mv $RECEIVE_DIR/$i $POSSIBLE_SUCCESS;
                        else
                                echo $(date +%F)": file $i is case 4 (single scenario)" >> $LOG_DIR/${NAMING_CONVENTION}.log;
                        ...
                        if grep -rq $(head -n1 $RECEIVE_DIR/$i | cut -f31 -d,) $CASE5_DIR; then
                                ...
                                echo $(date +%F)": file $i is case 3 (no middle)" >> $LOG_DIR/${NAMING_CONVENTION}.log;
                        ...
                        if grep -rq $(head -n1 $RECEIVE_DIR/$i | cut -f31 -d,) $CASE2_DIR; then
                                ...
                                echo $(date +%F)": file $i is case 1 (no end)" >> $LOG_DIR/${NAMING_CONVENTION}.log;
                ...
                if grep -rq $(head -n1 $RECEIVE_DIR/$i | cut -f31 -d,) $CASE1_DIR; then
                        echo $(date +%F)": file $i is a possible success for case1" >> $LOG_DIR/${NAMING_CONVENTION}.log;
                        mv $RECEIVE_DIR/$i $POSSIBLE_SUCCESS;
                elif grep -rq $(head -n1 $RECEIVE_DIR/$i | cut -f31 -d,) $CASE2_DIR; then
                        echo $(date +%F)": file $i is a possible success for case2" >> $LOG_DIR/${NAMING_CONVENTION}.log;
                        mv $RECEIVE_DIR/$i $POSSIBLE_SUCCESS;
                elif grep -rq $(head -n1 $RECEIVE_DIR/$i | cut -f31 -d,) $CASE3_DIR; then
                        echo $(date +%F)": file $i is a possible success for case3" >> $LOG_DIR/${NAMING_CONVENTION}.log;
                        mv $RECEIVE_DIR/$i $POSSIBLE_SUCCESS;
                elif grep -rq $(head -n1 $RECEIVE_DIR/$i | cut -f31 -d,) $CASE4_DIR; then
                        echo $(date +%F)": file $i is a possible success for case4" >> $LOG_DIR/${NAMING_CONVENTION}.log;
                        mv $RECEIVE_DIR/$i $POSSIBLE_SUCCESS;
                elif grep -rq $(head -n1 $RECEIVE_DIR/$i | cut -f31 -d,) $CASE5_DIR; then
                        echo $(date +%F)": file $i is a possible success for case5" >> $LOG_DIR/${NAMING_CONVENTION}.log;
                        mv $RECEIVE_DIR/$i $POSSIBLE_SUCCESS;
                        ...
                        echo $(date +%F)": file $i is case 5 (1 liner middle entry)" >> $LOG_DIR/${NAMING_CONVENTION}.log;
                        mv $RECEIVE_DIR/$i $CASE5_DIR;
                else
                        echo $(date +%F)": file $i is case 2 (no beginning)" >> $LOG_DIR/${NAMING_CONVENTION}.log;
                ...
                if grep -rq $(head -n1 $RECEIVE_DIR/$i | cut -f31 -d,) $CASE1_DIR; then
                        ...
                        echo $(date +%F)": file $i is case 6 (1 liner last entry)" >> $LOG_DIR/${NAMING_CONVENTION}.log;
                        mv $RECEIVE_DIR/$i $CASE6_DIR;

                fi;
        else
                echo $(date +%F)": file $i is out of current scope, moving to error handling directory" >> $LOG_DIR/${NAMING_CONVENTION}.log;
...
for i in $(ls $WLNFILTER_DIR/); do
        ...
        if ! grep -q $i $TSTAMP_DIR/urs_d_WireLine_Ftr_Report_tstamp.txt; then
                echo "$i $(stat -c %Y $WLNFILTER_DIR/$i) $(($(stat -c %Y $WLNFILTER_DIR/$i)+$R_LIFETIME)) $(head -n1 $WLNFILTER_DIR/$i | cut -f31 -d,)" >> $TSTAMP_DIR/urs_d_WireLine_Ftr_Report_tstamp.txt;
        fi;
...
for i in $(ls $WLNFILTER_ARCHIVE_DIR/); do
        ...
        if ! grep -q $i $TSTAMP_DIR/urs_d_WireLine_Ftr_Archive_tstamp.txt; then
                echo "$i $(stat -c %Y $WLNFILTER_ARCHIVE_DIR/$i) $(($(stat -c %Y $WLNFILTER_ARCHIVE_DIR/$i)+$R_LIFETIME)) $(head -n1 $WLNFILTER_ARCHIVE_DIR/$i | cut -f31 -d,)" >> $TSTAMP_DIR/urs_d_WireLine_Ftr_Archive_tstamp.txt;
        fi;
done
