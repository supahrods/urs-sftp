...
if [ $(ls $USAGE_DIR | grep .*\.txt | wc -w) == 0 ]; then
	...
	for i in $(cat $USAGE_DIR/*.txt 2> /dev/null | grep .*\.dat); do # Check every content of EOD file that ends with .dat
	...
	echo -e "------------------------- Detailed Summary Report ----------------------------------------------------------------" >> $REPORT_DIR/$DATE_FILENAME;
	...
	echo -e "--------- List of File(s) Without FIN/DAT ----------------------------" >> $REPORT_DIR/$DATE_FILENAME;
	echo $LIST_WOFIN | tr " " "\n" >> $REPORT_DIR/$DATE_FILENAME;
