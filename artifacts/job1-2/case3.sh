...
for i in $(ls $CASE3_DIR/ | grep .*.ftr$); do  ## for every file in case3 dir
	...
	if [ $(stat -c %Y $CASE3_DIR/$i) -lt $(cat $TSTAMP_DIR/timestamps.txt | grep $i | cut -f3 -d" ") ]; then
		## Check for success
		if grep -qr $(head -n1 $CASE3_DIR/$i | cut -f31 -d,) $POSSIBLE_SUCCESS; then
			for a in $(cat $BETWEEN_DIR/$i); do
				...
				if [ $(cat $BETWEEN_DIR/$i | wc -l) == 0 ]; then
					mv $(grep -r $(head -n1 $CASE3_DIR/$i | cut -f31 -d,) $POSSIBLE_SUCCESS | cut -f1 -d: | xargs readlink -f | uniq) $SUCCESS_DIR;
					sed -i "/$(head -n1 $CASE3_DIR/$i | cut -f31 -d,)/d" $TSTAMP_DIR/timestamps.txt;
					mv $CASE3_DIR/$i $SUCCESS_DIR;
				fi;
			done;
		fi;
	elif [ $(stat -c %Y $CASE2_DIR/$i) -ge $(cat $TSTAMP_DIR/timestamps.txt | grep $i | cut -f3 -d" ") ]; then
		sed -i "$ $(tail -n1 $CASE3_DIR/$i | cut -f27-31 -d,)/$(cat $CASE3_DIR/$i | wc -l),$(tail -n1 $CASE3_DIR/$i | cut -f28-31 -d,)/" $CASE3_DIR/$i;	
		sed -i "/$(head -n1 $CASE3_DIR/$i | cut -f31 -d,)/d" $TSTAMP_DIR/timestamps.txt;
		mv $CASE3_DIR/$i $SUCCESS_DIR;
	fi;
	rm $BETWEEN_DIR/$i;
